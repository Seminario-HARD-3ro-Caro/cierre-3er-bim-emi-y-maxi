...
void loop() {
  ...
  if(digitalRead(boton)==HIGH){ // si el boton esta presionado y un miliseguno despues no lo esta es porque se pulso y no se esta manteniendo apretado
    ...
    if(digitalRead(boton)==LOW){ 
      count+=1;
      if(count==16){ // si la cuenta pasa de 4 se reinicia para que funcione en loop
        count=0;
      }
      switch(count){ // CADA CASO ES UN NUMERO EN BINARIO --> el led azul debe estar a la izquierda del todo, a su derecha el rojo, luego el verde y por ultimo el amarillo
        case 0:
